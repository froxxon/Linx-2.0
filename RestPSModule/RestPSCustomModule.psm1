    ...
    if ($null -ne $Route) {
        ...
        if ($RequestCommand -match "\.ps1$") {
            ...
            if ($ScriptParams.Count -gt 0) {
                # Use splatting with @ to expand array elements as separate arguments
                # For "Script.ps1 refreshAll" -> @("refreshAll") expands to individual args
                # For "Script.ps1 -Type refreshAll" -> @("-Type", "refreshAll") expands correctly
...
function Invoke-SQLCommand {
    ...
    Invoke-SqlCmd -ServerInstance $SQLServer -Database $Database -Query $Command -TrustServerCertificate -MultiSubnetFailover
>>>>>>> 0fb95b5 (Fix: Use array splatting for route parameters)
}